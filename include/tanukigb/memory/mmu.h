...
class TANUKIGB_EXPORT MMU { // Could make two classes one for each....?
 public:
  ...
  static MMU ColourGameboyMMU() { return MMU(Bootrom::CGBRom()); }
   ...
   void Write(word_t addr, byte_t value);

 private:

   MMU(Bootrom&& bootrom)
      : bootrom_(std::move(bootrom)), psudo_ram_{std::make_unique<byte_t[]>(FULL_RAM_BUFFER_SIZE)} {}
   ...
   void DisableBootRom() {} //TODO: // { this->Write(BOOT_REGISTER_ADDRESS, 0x01); }
   ...
   //static constexpr int FULL_RAM_BUFFER_SIZE = 0x10000; // 65,536
   static constexpr size_t FULL_RAM_BUFFER_SIZE = 0x10000;  // 65,536
